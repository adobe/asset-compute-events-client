...
class AdobeIOEvents {
    ...
    async registerEventProvider(provider, retryOptions=true) {
        ...
        try {
            ...
            console.error(e);
            if(e.code === 409){
                console.log(`Event provider for ${provider.consumerOrgId}, ${provider.projectId}, ${provider.workspaceId} already exists`);
            }
            throw e;
        }
    }
    ...
    async registerEventType(provider, eventType, retryOptions=true) {
        // api-register does not pass the retry options so it would default to retry by default
        // TODO: handle retry options to api-register
        const httpOptions = {
            timeout: parseInt(process.env.NODE_FETCH_RETRY_SOCKET_TIMEOUT) || DEFAULT_SOCKET_TIMEOUT,
        ...
        try {
            ...
            const result = await aioLibEventClient.createEventMetadataForProvider(this.consumerOrgId, this.projectId, this.workspaceId, provider.id, body);
            console.log(result);
            return result;
    ...
    async listConsumerRegistrations(consumerId, applicationId, retryOptions=true) {
        ...
        // call methods
        try {
            const result = await aioLibEventClient.getAllRegistrationsForWorkspace(this.consumerOrgId, this.projectId, this.workspaceId);
            console.log(result);
