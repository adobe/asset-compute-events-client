        ...
        try {
            ...
            if(result && result._embedded && result._embedded.providers 
                ...
                providers.forEach(provider => {
                    // although journals were found, we must make sure they are enabled
                    if (Array.isArray(provider._embedded.eventmetadata)) {
                        console.log('provider._embedded.eventmetadata ', provider._embedded.eventmetadata);
                        const hasProvider = provider._embedded.eventmetadata.some(eventMeta => eventMeta !== null && 
                                eventMeta.evet_code === AdobeIOEvents.Metadata.ASSET_COMPUTE);
