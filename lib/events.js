...
class AdobeIOEvents {
    ...
    constructor(options) {
        ...
        this.consumerOrgId = options.consumerId;
    ...
    async registerEventProvider_AIOLIBEVENTS(provider, retryOptions=true) {
        ...
        try {
            ...
            const result = await aioLibEventClient.createProvider(provider.consumerOrgId, provider.projectId, provider.workspaceId, body);
            console.log(result);
    ...
    async registerEventType(eventType, retryOptions=true) {
        ...
        const providerId = eventType.provider || this.defaults.providerId;
        const url =`${IO_API_HOST[this.env]}/${this.consumerOrgId}/${this.projectId}/${this.workspaceId}/providers/${providerId}/eventmetadata`;
        // POST /{consumerOrgId}/{projectId}/{workspaceId}/providers/{providerId}/eventmetadata
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'x-ims-org-id': this.auth.orgId,
