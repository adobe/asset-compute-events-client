...
class AdobeIOEvents {
     ...
     */
    constructor(options) {
        console.log(`ioevents constuctor`)
    ...
    async getEventProvider(consumer, retryOptions=true) {
        const consumerOrgId = consumer || this.consumerOrgId || this.defaults.consumerOrgId;
            ...
            if(result && result._embedded && result._embedded.providers 
                ...
                providers.forEach(provider => {
                    ...
                    if (provider.provider_metadata === AdobeIOEvents.Metadata.ASSET_COMPUTE && 
                            provider.instance_id.includes(this.auth.clientId) &&
     ...
     * @returns {Promise}
     */
    async deleteEventProvider_OLD(providerId, retryOptions=true) {
    ...
    async deleteEventProvider(options, retryOptions=true) {
        const consumerOrgId = options.consumerOrgId || this.consumerOrgId;
        ...
        console.log(`delete event provider`)
        console.log(`consumerOrgId: ${consumerOrgId}`)
        ...
        // call methods
        try {
            const result = await aioLibEventClient.deleteProvider(consumerOrgId,
    ...
    async sendEvent(event, retryOptions) {
        ...
        console.log(`sending event to ${url} with payload ${payload}`);
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    async listConsumerRegistrations(consumerId, project, workspace, retryOptions=true) {
        console.log(`list consumer registrations`)
        ...
        try {
            ...
            console.log('result is :', result);
            console.log('result._embedded is :', result._embedded);
            if(result && result._embedded && result._embedded.registrations) {
                console.log('All Registrations found', result._embedded.registrations);
     ...
     * @returns {Promise}
     */
    async deleteJournal(consumerId, projectId, workspaceId, registrationId, retryOptions=true) {
        ...
        const url = `${IO_API_HOST[this.env]}/events/${consumerId}/${projectId}/${workspaceId}/registrations/${registrationId}`;
        const response = await fetch(url, {
            method: 'DELETE',
            headers: {
                'x-ims-org-id': this.auth.orgId,
